...
public class TagController {
    ...
    @GetMapping("/tags")
    public String listTag(@PageableDefault(size = 10, sort = {"id"}, direction = Sort.Direction.DESC) Pageable pageable,
                       Model model) {
    ...
    public String editPost(@Valid Tag type, BindingResult result,@PathVariable Long id, RedirectAttributes attributes){
        Tag t1 = tagService.getTagByName(type.getName());
        if (t1 != null){
            result.rejectValue("name", "nameError", "不能添加重复分类");
        }
        if (result.hasErrors()){
        ...
        Tag t2 = tagService.updateTag(id,type);
        if (t2 == null){
